...
void update_state_handler(Command_t * cmd, size_t arg_idx){
	...
	while(arg_idx < cmd->args_len){
		if(!strncmp(cmd->args[arg_idx], "from", 4)){
			table_state_handler_upd(cmd, arg_idx+1);
			return;
...
void set_state_handler(Command_t *cmd, size_t arg_idx){
	if(arg_idx < cmd->args_len){
		...
		if(!strncmp(str, "id", 2)){
			idx += 2;
			cmd->cmd_args.upd_args.field = strdup("id");
		}else if(!strncmp(str, "name", 4)){
			idx += 4;
			cmd->cmd_args.upd_args.field = strdup("name");
		}else if(!strncmp(str, "email", 5)){
			idx += 5;
			cmd->cmd_args.upd_args.field = strdup("email");
		}else if(!strncmp(str, "age", 3)){
			idx += 3;
			cmd->cmd_args.upd_args.field = strdup("age");
		}
		...
		cmd->cmd_args.upd_args.literal = strdup(str+idx);

		arg_idx++;
		if(arg_idx == cmd->args_len)return;
		else if(!strncmp(cmd->args[arg_idx], "where", 5)){
			where_state_handler_upd(cmd, arg_idx+1);
			return;
		}
	}
	cmd->type = UNRECOG_CMD;
...
void where_state_handler_upd(Command_t * cmd, size_t arg_idx){
	if(arg_idx < cmd->args_len){
		...
		if(!strncmp(str, "and", 3)){
			...
			where_state_handler_upd(cmd, ++arg_idx);
