...
///
///
int update_user(Table_t *table, size_t idx, User_t *user){
    memcpy((table->users)+idx, user, sizeof(User_t));
    return 1;
}
...
///
///
int delete_user(Table_t *table, size_t idx){
	size_t i;
	for(i = idx+1; i < table->len; i++)get_User(table, i);
	User_t *tmp = (User_t*)malloc(sizeof(User_t)*table->len-idx-1);
	memcpy(tmp, (table->users)+idx+1, sizeof(User_t)*(table->len-idx-1));
    	memcpy((table->users)+idx, tmp, sizeof(User_t)*(table->len-idx-1));
	free(tmp);
	table->cache_map[table->len-1] = 0;
	table->len--;
	if(table->fp != NULL){
		archive_table(table);
		load_table(table, table->file_name);
	}
	return 1;
}
...
int archive_table(Table_t *table) {
    size_t archived_len = 0;
    //struct stat st;
    size_t idx;
    for(idx = 0; idx < table->len; idx++)get_User(table, idx);
    ...
    fclose(table->fp);
    table->fp = fopen(table->file_name, "w+b");
