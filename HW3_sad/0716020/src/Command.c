...
int add_where_condition(Command_t *cmd, const WhereArgs_t * argument){
	size_t where_len = cmd->cmd_args.where_len;
	WhereArgs_t *buf = (WhereArgs_t*)malloc(sizeof(WhereArgs_t) * (where_len+1));
	if(buf == NULL)return 0;
	if(cmd->cmd_args.whe_args){
		memcpy(buf, cmd->cmd_args.whe_args, sizeof(WhereArgs_t) * where_len);
		free(cmd->cmd_args.whe_args);
	}

	cmd->cmd_args.whe_args = buf;
	cmd->cmd_args.whe_args[where_len++] = *argument;
	cmd->cmd_args.where_len = where_len;
	return 1;
}
...
void cleanup_Command(Command_t *cmd) {
	...
	free(cmd->cmd_args.upd_args.field);
	free(cmd->cmd_args.upd_args.literal);
	cmd->cmd_args.upd_args.field = NULL;
	cmd->cmd_args.upd_args.literal = NULL;
